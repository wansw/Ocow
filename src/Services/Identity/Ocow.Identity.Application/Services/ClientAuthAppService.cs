...
public class ClientAuthAppService : IClientAuthAppService
{
    ...
    /// 创建小程序认证应用服务。
    /// </summary>
    public ClientAuthAppService(IIdentityRepository repository, ITokenService tokenService, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<AuthTokenResDto> WechatLoginAsync(WechatLoginReqDto reqDto, CancellationToken cancellationToken = default)
    {
        return await _unitOfWork.ExecuteInTransactionAsync(async tokenCancellationToken =>
        {
            ...
            var identity = await _repository.GetMemberIdentityByOpenIdAsync(openId, tokenCancellationToken);
            ...
            if (identity is null)
            {
                ...
                await _repository.SaveMemberIdentityAsync(identity, tokenCancellationToken);
            }
